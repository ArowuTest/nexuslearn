...
Status: **partially implemented** across the canonical grading remediation batches. The transaction boundary, typed/versioned response path, durable deduplicated contract snapshots, mock separation, recovery tests, public answer-key decoupling and retirement of new legacy submissions are implemented. Explicit exact alternatives, case sensitivity and bounded numerical tolerances are implemented in the September 7 authored-policy follow-on; units, semantic/rubric marking and other limits below remain open. See `docs/reviews/2026-09-06-canonical-grading-batch.md` and the follow-on evidence below for limits. Follows G01/G06 in the September game audit and mission integrity/access repair batch.

Follow-on: public mission-question decoupling is implemented with acceptance evidence in `docs/reviews/2026-09-06-pupil-question-contract.md`; private generated-report delivery is recorded in `docs/reviews/2026-09-06-private-content-reports.md`. Adult recent-attempt provenance is implemented in `docs/reviews/2026-09-06-adult-attempt-evidence.md`, with hosted release status in the FS V2B checkpoint. That reporting scope is a latest-10 frozen-evidence preview, not an all-time audit. New legacy submissions are retired while completed historical retries retain their stored result; semantic-review markers now fail closed. See `docs/reviews/2026-09-06-grading-contract-retirement.md`. Original typed submissions and correctness-grader revision are now archived for new attempts, with adult report display and no historical backfill: `docs/reviews/2026-09-07-submitted-answer-evidence.md`. Richer marking policies, reward/mastery-policy snapshots, subject-specific incorrect feedback and full-history audit remain open.
...
September 7 authored-policy follow-on: see `docs/content-marking-policies.md` for the versioned authoring contract and `docs/reviews/2026-09-07-authored-marking-policies.md` for tests and boundaries. This closes the explicit alternatives/tolerance/case-sensitive subset of steps 4–5, without inferring unit conversions or relaxing moderation. Legacy spoken-punctuation annotations do not alter written marking. Curriculum status and human review gates are unchanged.
