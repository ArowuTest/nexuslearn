...
- Hosted CI and exact GitHub/Vercel/Render revision verification follow the release commit. Local verification is not a claim that the new revision is already deployed.

## Remaining boundaries

Some teaching operations still require raw objective IDs; school/group/learner overview pagination and task-specific selectors need further work. Mobile admin navigation, broader teacher/tutor workflows and shared-device adult-to-pupil handoff remain separate follow-ups. This card-session observer does not claim to retrofit every other cached private workspace.
