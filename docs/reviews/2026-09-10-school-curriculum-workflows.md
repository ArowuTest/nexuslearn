...
Hosted revision `055acbd6aa96c17c28acf372da6e715bde64f227` passed the exact-SHA smoke on both Vercel and Render. GitHub Content quality `34424057458` passed. Platform quality `34424057535` web job passed **420 browser cases including Linux visual baselines**, followed by **6 real grading and 6 real role cases**, with no flaky/retry summary. Its API job failed in shared integration setup: `pgcrypto` was being installed in a disposable test schema, so concurrent packages could race extension creation and cleanup.

The follow-up reproducer failed deterministically on a fresh local database by showing `pgcrypto` owned by a temporary school-test schema. The shared test bootstrap now installs it in `public` under a transaction-scoped advisory lock before any isolated schema migrations. All four PostgreSQL test harnesses use it. The reproducer then passed, and the full API suite passed (learning 97.847s, server 53.683s), followed by vet and build. This changes test infrastructure only; application migrations, schema isolation, test assertions and CI parallelism remain intact. Follow-up GitHub acceptance remains required.

Additional fresh-database stress ran the curriculum/ownership cases in both learning and server packages concurrently for three repetitions: learning 65.270s and server 48.571s, both passed. This starts from a database with no preinstalled extension and does not serialize the test packages to hide the race.
