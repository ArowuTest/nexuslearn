...
test("real family signup opens an empty workspace ready for its first child", async ({ page }, info) => {
  ...
  expect(saved.engagement.version).toBeGreaterThan(0);
  const parentHeaders = { Authorization: childResponse.request().headers().authorization };
  const supportURL = `${api}/v1/parent/children/${childId}/engagement`;
  const supportDraft = { ...saved.engagement, notes: "Keep the more recent family support." };
  const newerSupport = await page.request.put(supportURL, { headers: { ...parentHeaders, "Idempotency-Key": `family-support-${info.project.name}` }, data: supportDraft });
  expect(newerSupport.status()).toBe(200);
  const newerProfile = await newerSupport.json();
  expect(newerProfile.version).toBeGreaterThan(saved.engagement.version);
  const setupRetry = await page.request.put(`${api}/v1/parent/children/${childId}`, { headers: parentHeaders, data: childResponse.request().postDataJSON() });
  expect(setupRetry.status()).toBe(200);
  const retriedChild = await setupRetry.json();
  expect(retriedChild.engagement.version).toBe(newerProfile.version);
  expect(retriedChild.engagement.notes).toBe(supportDraft.notes);
  expect(retriedChild.credential).toEqual(saved.credential);
  const supportRetry = await page.request.put(supportURL, { headers: { ...parentHeaders, "Idempotency-Key": `family-support-${info.project.name}` }, data: supportDraft });
  expect(supportRetry.status()).toBe(200);
  expect((await supportRetry.json()).save_result).toEqual({ applied_version: newerProfile.version, changed: true, replayed: true });
...
test("real school creates a class, pupil, teaching group and usable login card", async ({ page }, info) => {
  ...
  const firstSupportResponse = await profileSaved;
  expect(firstSupportResponse.status()).toBe(200);
  const firstSupport = await firstSupportResponse.json();
  expect(firstSupport.learning_approaches).toContain("reduced_motion");
  expect(firstSupport.save_result.applied_version).toBe(firstSupport.version);
  await responseSettled(page, profileSaved);
  await expect(page.getByRole("status")).toContainText("saved");
  // A second editor's authenticated write reaches real PostgreSQL while this
  // browser still holds its earlier base. The stale browser must not win.
  const supportURL = `${api}/v1/school/students/${pupilId}/engagement`;
  const authorization = firstSupportResponse.request().headers().authorization;
  const elsewhere = await page.request.put(supportURL, { headers: { Authorization: authorization, "Idempotency-Key": `elsewhere-${info.project.name}` }, data: { ...firstSupport, notes: "Newer saved school support." } });
  expect(elsewhere.status()).toBe(200);
  const latestSupport = await elsewhere.json();
  expect(latestSupport.version).toBeGreaterThan(firstSupport.version);
  const supersededRetry = await page.request.put(supportURL, { headers: { Authorization: authorization, "Idempotency-Key": firstSupportResponse.request().headers()["idempotency-key"] }, data: firstSupportResponse.request().postDataJSON() });
  expect(supersededRetry.status()).toBe(409);
  expect(await supersededRetry.json()).toMatchObject({ code: "support_profile_conflict", previously_saved: true, current_profile: { version: latestSupport.version } });
  const notes = page.getByRole("textbox", { name: "Operational notes", exact: true });
  await notes.fill("My reviewed school draft.");
  const staleSave = page.waitForResponse(r => r.url().endsWith(`/${pupilId}/engagement`) && r.request().method() === "PUT");
  await page.getByRole("button", { name: "Save support profile", exact: true }).click();
  expect((await staleSave).status()).toBe(409);
  await expect(notes).toHaveValue("My reviewed school draft.");
  await expect(page.getByRole("button", { name: "Save support profile", exact: true })).toBeDisabled();
  const comparison = page.getByRole("region", { name: "Review changed support settings" });
  await expect(comparison).toContainText("Newer saved school support.");
  await comparison.scrollIntoViewIfNeeded();
  await capture(page, info, "06-school-support-conflict");
  await page.getByRole("button", { name: "Review my draft against these settings", exact: true }).click();
  const resolvedSave = page.waitForResponse(r => r.url().endsWith(`/${pupilId}/engagement`) && r.request().method() === "PUT");
  await page.getByRole("button", { name: "Save support profile", exact: true }).click();
  expect((await resolvedSave).status()).toBe(200);
  const resolvedProfile = await (await resolvedSave).json();
  expect(resolvedProfile.notes).toBe("My reviewed school draft.");
  expect(resolvedProfile.learning_approaches).toContain("reduced_motion");
  expect(resolvedProfile.version).toBeGreaterThan(latestSupport.version);
