...
function ContextChoiceBoard({ question, input, onChoose, onSubmit }: { question: StudioQuestion; input: string; onChoose: (value: string) => void; onSubmit: () => void }) {
  ...
  return <section className="studio-workspace-panel" aria-label={title}>
    ...
    <button type="button" onClick={onSubmit} disabled={!input} className="btn-pop studio-check-answer" aria-label="Submit contextual answer">Send answer</button>
  </section>;
}
function StructuredChoiceBoard({ question, input, onChoose, onSubmit }: { question: StudioQuestion; input: string; onChoose: (value: string) => void; onSubmit: () => void }) {
  ...
  return <section className="studio-workspace-panel" aria-label={title}>
    <p className="studio-step-label">{title}</p><p className="mt-2 text-center text-sm text-white/80">{instruction}</p>
    ...
    <button type="button" onClick={onSubmit} disabled={!input} className="btn-pop studio-check-answer" aria-label="Submit structured answer">Send answer</button>
  </section>;
}
