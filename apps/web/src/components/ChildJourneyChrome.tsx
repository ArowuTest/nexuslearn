  ...
  return (
    <header className="relative z-20 mx-auto max-w-6xl rounded-2xl border border-white/12 bg-[#17233f]/80 px-4 py-3 shadow-[0_16px_50px_rgba(0,0,0,0.18)] backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href={backHref} className="shrink-0 font-display text-lg font-semibold text-white" aria-label="NexusLearn home">
          NexusLearn
        </Link>
        <div className="order-3 w-full min-w-0 text-center sm:order-none sm:w-auto sm:flex-1 sm:px-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          <Link href={backHref} onClick={onBack} data-switch-extra={scanBack || undefined} className="inline-flex min-h-11 items-center rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white">
            {backLabel}
          </Link>
