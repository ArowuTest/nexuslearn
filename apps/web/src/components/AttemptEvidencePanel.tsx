          ...
          <details key={item.id} className="min-w-0 rounded-lg border border-[#15213d]/15 p-3">
            ...
            <dl className="mt-3 grid min-w-0 gap-3 break-words [overflow-wrap:anywhere]">
              {[
                ...
                ["Support and response", `${item.independent ? "Independent" : "Supported; answer-revealing help recorded"} · ${item.assistance_used?.length ? item.assistance_used.join(", ").replace("hint", "Hint used") : item.hint_used ? "Hint used" : "No support recorded"} · ${item.response_mode || "Response method unavailable"} · ${item.format}`],
