...
export default function SchoolLearningTask({ kind, learner, disabled, request, onSaved, onBusyChange }: {
  ...
  const resetDraft = useCallback(() => { setDraft(emptyDraft(kind)); setObjective(null); }, [kind]);
  const { message, failed, refreshFailed, expired, save: persist, refresh } = useSchoolSave({ disabled, request, onSaved, onBusyChange, resetDraft, savedMessage: config.saved });
  ...
  async function save() {
    if (!valid || !learner || !objective) return;
    ...
    await persist({ path: `/v1/school/${config.path}`, body: JSON.stringify(payload), pupil: learner.external_ref });
  }
