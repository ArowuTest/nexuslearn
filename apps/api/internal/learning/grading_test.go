...
func TestLegacyNumericEvidenceMustActuallyBePresent(t *testing.T) {
	...
	for _, suffix := range []string{"", `,"given":null`, `,"given":0`} {
		...
		if suffix == `,"given":0` {
			if err != nil || !result.Correct {
				t.Fatalf("explicit zero rejected: %+v %v", result, err)
			}
		} else if !errors.Is(err, ErrInvalidResponse) {
			t.Fatalf("absent numeric evidence accepted %s: %+v %v", suffix, result, err)
		}
	}
}

func TestLegacyPresenceTrackingDoesNotChangeHistoricalReplayHash(t *testing.T) {
	var decoded Attempt
